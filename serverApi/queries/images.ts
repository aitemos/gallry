...
/*const ensureImages = async () => {
  ...
  if (!collectionsNames.includes('images')) {
    const col = getCollection<ImageEntity>('images')  
      await col.add({code:Image({id:"1",src:})});
...
export const images = async (_parent: any, _args: any, _context: any):Promise<Image[]> => {
  //await ensureImages();
  const query = getCollection<ImageEntity>('images');
